...
export class ImageProcessor extends BaseProcessor {
  async process(inputPath: string, outputPath: string, request: MediaRequest): Promise<ProcessingResult> {
    ...
    const inputArgs = [
      '-y', // Overwrite output
      '-f', 'lavfi',
      '-i', `color=size=${canvas.width}x${canvas.height}:color=${request.background.color}`, // Canvas [0]
    ...
    const canvasOverlay = this.buildCanvasOverlay(request, canvas, mainContentFilter.outputTag);
